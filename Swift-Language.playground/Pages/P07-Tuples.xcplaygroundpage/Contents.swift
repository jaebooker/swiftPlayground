...
var endCoord: (x: Double, y: Double) = (375.0, 667.0)
var pythag = sqrt((endCoord.x - startCoord.x)*(endCoord.x - startCoord.x) + (endCoord.y - startCoord.y)*(endCoord.y - startCoord.y))
print(pythag)
var currency: (type: String, amount: Double) = ("USD", 1.00)
var CAD: (type: String, amount: Double) = ("CAD", 1.29)
var AUS: (type: String, amount: Double) = ("AUS", 1.36)
print("\(CAD.type) is worth \(CAD.amount) of \(currency.type), while \(AUS.type) is worth \(AUS.amount) of \(currency.amount)")
