/*:
 ![Make School Banner](./swift_banner.png)
# Challenge!
To cap things off, here's a challenge that will require you to use many of the concepts you've learned so far. You're going to be creating a simple card game, in which you (the player) draw a card against the computer. The highest card wins! What an exciting game!
*/
/*:
## Card Class
To start we'll make a simple card class. In fact, instead of a class, it might be even better to create a card `struct`. A `struct` is a little bit better for a couple reasons:
 
1. We don't expect to inherit from the card class, so `struct`s lack of inheritance is okay
2. A `struct` will be a little bit more performant, because of the small amount of memory our card will occupy.
 
 If all that seems a little vague or weird, that's okay! Choosing a `struct` instead of a `class` or vice-versa won't actually matter too much in any case.
 
 Our card struct will have two properties: `rank` and `suit`. There are 13 ranks: Ace, King, Queen, Jack and Ten through Two. And, of course, the 4 suits are Spades, Clubs, Diamonds and Hearts.
 
 There's a couple of ways we could represent this information. For example, we could use an `Int` to represent the rank, Two would be `2`, Ten would be `10`, Jack would be `11`, etc. Or maybe we could use strings, like: `"Ace"`, `"Queen"`, `"Diamonds"`. But we shouldn't do either of those things - there's a better way! Can you think of what it is?

Hopefully the answer you came up with is *enumerations*. Go ahead and use the following code to create your enumerations:
 
    enum Rank {
        case two, three, four, five, six, seven, eight, nine, ten, jack, queen, king, ace
        
        static func allValues() -> [Rank] {
            return [two, three, four, five, six, seven, eight, nine, ten, jack, queen, king, ace]
        }
    }

    enum Suit {
        case spades, hearts, diamonds, clubs
        
        static func allValues() -> [Suit] {
            return [spades, hearts, diamonds, clubs]
        }
    }
 
Both enumerations come with an `allValues()` function, which returns an array containing each of the enumeration values. This function will be helpful later, when you create your `Deck` class.

 
So now, using the above `enum` code (you'll have to type it out below), create a Card `struct` with `suit` and `rank` properties, and an initializer that accepts `suit` and `rank` parameters.

You'll probably also find the following function useful in this exercise. It allows you to generate a random number between `0` and `upper - 1`. Please do note, the random number will *never* equal upper.
*/

import Foundation
func randomInteger(upper: Int) -> Int {
    return Int(arc4random_uniform(UInt32(upper)))
}

/* Place your code here! */
enum Rank: Int {
    case two = 0, three, four, five, six, seven, eight, nine, ten, jack, queen, king, ace, wild
    
    static func allValues() -> [Rank] {
        return [two, three, four, five, six, seven, eight, nine, ten, jack, queen, king, ace]
    }
}

enum Suit: Int {
    case spades = 0, hearts, diamonds, clubs, joker
    
    static func allValues() -> [Suit] {
        return [spades, hearts, diamonds, clubs]
    }
}


/*:
## Deck Class
 
Now that you've created your card struct, it's time to create a `Deck` to hold them. A standard 52 card deck has one card of each rank and suit pair. The cards in a deck must also be ordered: can you think of a data structure to hold them? How will you generate all the possible card combinations? Maybe a nested for loop ;)
 
To start, your deck should have an empty initializer. Inside the initializer, create all 52 cards, and place them in the ordered data structure.
*/
/* Place your code here! */
//var cardDeck: [Suit] = []
//var deck: [Rank: Suit] = [:]
var tupleDeck: [(rank: Rank, suit: Suit)] = []
func NewDeck() {
    for n in 0...53 {
        for i in 0...3 {
            if i == 0 {
                tupleDeck.append((rank: Rank.two, suit: Suit.spades))
                tupleDeck.append((rank: Rank.three, suit: Suit.spades))
                tupleDeck.append((rank: Rank.four, suit: Suit.spades))
                tupleDeck.append((rank: Rank.five, suit: Suit.spades))
                tupleDeck.append((rank: Rank.six, suit: Suit.spades))
                tupleDeck.append((rank: Rank.seven, suit: Suit.spades))
                tupleDeck.append((rank: Rank.eight, suit: Suit.spades))
                tupleDeck.append((rank: Rank.nine, suit: Suit.spades))
                tupleDeck.append((rank: Rank.ten, suit: Suit.spades))
                tupleDeck.append((rank: Rank.jack, suit: Suit.spades))
                tupleDeck.append((rank: Rank.queen, suit: Suit.spades))
                tupleDeck.append((rank: Rank.king, suit: Suit.spades))
                tupleDeck.append((rank: Rank.ace, suit: Suit.spades))
            }
            else if i == 1 {
                tupleDeck.append((rank: Rank.two, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.three, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.four, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.five, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.six, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.seven, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.eight, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.nine, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.ten, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.jack, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.queen, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.king, suit: Suit.hearts))
                tupleDeck.append((rank: Rank.ace, suit: Suit.hearts))
            }
            else if i == 2 {
                tupleDeck.append((rank: Rank.two, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.three, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.four, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.five, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.six, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.seven, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.eight, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.nine, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.ten, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.jack, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.queen, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.king, suit: Suit.diamonds))
                tupleDeck.append((rank: Rank.ace, suit: Suit.diamonds))
            } else if i == 3 {
                tupleDeck.append((rank: Rank.two, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.three, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.four, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.five, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.six, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.seven, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.eight, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.nine, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.ten, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.jack, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.queen, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.king, suit: Suit.clubs))
                tupleDeck.append((rank: Rank.ace, suit: Suit.clubs))
            } else {
                print("This is wrong")
            }
        }
    }
}

/*:
Now that you have a `Deck` it's time to add some functionality. Create a a function in your `Deck` class above called `drawOne()`. `drawOne()` should return a random card from the deck. Don't forget to remove that card from your internal deck data structure! 
  
Once you've implemented `drawOne()`, uncomment the following code to test it out! You can show the debug area (⇧⌘Y) to see what's printed.
*/

func drawOne() -> (rank: Rank, suit: Suit) {
    if tupleDeck.count != 0 {
        var index: Int = Int(arc4random_uniform(UInt32(tupleDeck.count)))
        var card = tupleDeck[index]
        tupleDeck.remove(at: index)
        print(card)
        return card
    } else {
        tupleDeck.append((rank: Rank.wild, suit: Suit.joker))
        var card = tupleDeck[0]
        print("Oh no! You got the Joker! You must be out of cards. Hit NewDeck to start over")
        return card
    }
}
NewDeck()
drawOne()
//func printCard(_ card: card?) {
//    drawOne()
//    if let card = card {
//        print("The random card is the \(card.rank) of \(card.suit)")
//    } else {
//        print("That's not a card!")
//    }
//}

//var testDeck = Deck()

//for _ in 1...55 {
//    let card = testDeck.drawOne()
//    printCard(card)
//}

/*:
- important:
 Does the test code above create an error?  If so, it's likely because you forgot to account for the case where there's no cards left in the deck to draw! Change your `drawOne()` method so that it returns `nil` if there's no cards left to draw.
 */
/*:
## A Simple Game
 Now it's time to make a simple game. Create a deck, and draw one card for you (the player) and one card for the computer. The highest card wins! In this game, a Two is the lowest card and Ace is the highest card. If both players draw the same rank card, then suit is used to determine the winner. Spades is the best suit, followed by Hearts, Diamonds, then Clubs.
 
 You should print out the result of the game like this:
    
>The (_player_ or _computer_) won with the _rank_ of _suit_!
 
 So, an example of the output might be:
 
    The player won with the King of Clubs!
 
 
 Here's some hints:
 
 It's best to make your code reusable. One way to do that is to place it into functions. I suggest you create a function to compare two cards to determine which one is the winner. It would also be good to have a function that prints the result of the game!
 
 You can make comparing the cards easier if you assign _raw values_ to the enumerations. Check out the [enumerations playground](P10-Enumerations) for a refresher on how to do that.
 */
/* Place your code here! */
func simpleGame() {
    var you = drawOne()
    var comp = drawOne()
   if you.rank.rawValue > comp.rank.rawValue {
        print("Oooh wee! You win! Nice play, with that \(you.rank) of \(you.suit)")
    }
   else if you.rank.rawValue < comp.rank.rawValue {
        print("Oooh weee! The computer cheated! Or you're a loser, not seeing that \(comp.rank) of \(comp.suit) coming in time, oooh weee")
    }
   else {
    if you.suit.rawValue > comp.suit.rawValue {
        print("Oooh Wee! That was a close one! Good thing you had that \(you.rank) of \(you.suit) up your sleeve, ooh wee!")
        }
    else if you.suit.rawValue < comp.suit.rawValue {
        print("You almost had it! What were you thinking?! He was playing a \(comp.rank) of \(comp.suit)! A freaking \(comp.rank) of \(comp.suit)!! OOOH WEEEE!")
        }
    else {
        print("Oooh weee! According to the program, I shouldn't exist. But here I am! Oooh wee!")
        }
    }
}


/*:
 - important:
 Once you're done, have your instructor check your code!
 */
/*:
[Previous](@previous) | [Table of Contents](P00-Table-of-Contents) | [Advanced Topics](@next)
 */
